...
export function EmployeeTable({ employees, period, showDepartment = true, highlightId }: {
  employees: Employee[]; period: ReviewPeriod; showDepartment?: boolean; highlightId?: string;
}) {
  return (
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {employees.map((e) => {
            ...
            return (
              <tr key={e.id} className={`border-t border-hair ${highlightId === e.id ? "bg-sky-bg/40" : ""}`}>
                <td className="px-4 py-3">
                  <Link href={`/employees/${e.id}?period=${period.id}`} className="flex items-center gap-3 group">
                    ...
                    <span>
                      ...
                      {highlightId === e.id && <span className="ml-2 text-[11px] text-muted">You</span>}
                      <span className="block text-xs text-muted">{e.designation || "—"}</span>
                    </span>
                  </Link>
                </td>
                {showDepartment && <td className="px-4 py-3 text-muted">{db.department(e.department_id)?.name}</td>}
          ...
          {!employees.length && (
            <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-muted">No employees here yet.</td></tr>
          )}
        </tbody>
      </table>
    </div>
